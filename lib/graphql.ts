...
export async function getProductById(id: string): Promise<{
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const productFound = mockProducts.find((p) => p.id === id);

      if (!productFound) {
        resolve({ data: { product: null } });
        return;
      }

      // Add extra data only to the found product
      const productWithExtraData = {
        ...productFound,
        sku: `SKU-${productFound.id}`,
        inventory: Math.floor(Math.random() * 100),
        manufacturer: "Generic Brand",
        weight: Math.random() * 5,
        dimensions: { width: 10, height: 10, depth: 10 },
      ...
      resolve({ data: { product: productWithExtraData } });
    }, 500);
  });
}
